...
router.post("/borrow-request/:id", async (req, res) => {
  try {
    ...
    const user = await query("insert into requested_book set ?", [requestData]);
...
router.get("/view-books", async (req, res) => {
  ...
  if (req.query.search) {
    ...
    if (isIntegerString(key)) {
      search = `where ISBN = ${parseInt(key)} or  rack_number = ${parseInt(
        key
      )}`;
    } else {
